...
**A window inside the date** (#282): `14:00 – 15:00`, never a clock time the customer types. Off for every shop until its owner turns `slots_enabled` on in the Fulfilment tab; while off, `hours`, `slot_minutes` and `slot_notice_minutes` sit dormant in the bag like an unused mode's settings, and a slot in a request body is **ignored**, not refused — a customer who loaded the page before the merchant flipped the switch must not be refused for it.

The rule is `selectableSlots` / `isSlotSelectable` in `packages/shared/src/fulfilment.ts`, beside the date pair and for the same reason. `hours` is a 7-entry array, one `open`/`close` per weekday (index 0 = Sunday), shared by pickup and delivery; a weekday with `null` hours is closed. **The Fulfilment tab shows one weekday control at a time**: with slots on, the hours editor's checkboxes are the closed days and the "Closed days" card is hidden; on save `closed_weekdays` is written from the unticked rows, so the row holds one truth and a shop that turns slots off later keeps the days it closed. Flipping the switch carries the days across in both directions. Slots step from `open` in `slot_minutes` (30 / 60 / 120), and a slot that runs past `close` is dropped. A slot must **start** at or after `now + slot_notice_minutes` on the shop's clock, counted in minutes from the shop's midnight today so a day of notice crosses midnight without a zone-offset calculation. **With slots on, a date with no slot is not offered** — `selectableDates` drops it and `isDateSelectable` refuses it — so the storefront never shows a day the customer cannot complete, and today greys out once its last slot has passed.

Intake judges the slot after the date and before the counter moves: `fulfil_time_required` (slots on, no slot) and `fulfil_time_unavailable` (outside hours, off the step grid, wrong length, too close to now, or one end without the other). The row stores **both ends** (`fulfil_time_from`, `fulfil_time_to`, Postgres `time`), not a start plus a length: a later change to `slot_minutes` must not rewrite what a customer was promised. Both null means "placed with no slot", the same rule `fulfil_date` uses. Postgres hands `time` back as `HH:MM:SS`; every reader brings it to `HH:MM` (a slice in the browser and the notifiers, `to_char` in `patchOrder`'s read).

The merchant may move a slot under the customer's rule (`judgeSlotPatch`, `apps/backend/src/orderSlotPatch.ts`, pure and unit-tested), judged against the date the row **will** hold — a moved date carries its slot along and the slot must be open on the new day, or the PATCH is refused with `fulfil_time_unavailable` and the drawer asks for a new slot. A slot shop's order can move its slot but never clear it; a shop with slots off can only clear one. Each move is a `fulfil_time_changed` event with both ends as `'HH:MM-HH:MM'` labels. The customer is not told, as with the date.

`validateSlotHours` refuses a save with `invalid_time` (an open day whose time is not `HH:MM` — an emptied input) or `close_before_open` — judged on the **raw** hours, because `fulfilmentConfig` reads such a day as closed and the parsed config can never show the mistake — or `no_open_day`, the slot twin of the all-seven-days-closed refusal. `fulfilmentWarning` reports `no_slots` (slots on, dates offered, none with a slot) in red on the dashboard. See [ADR 0027](docs/adr/0027-a-fulfilment-slot-is-a-window-the-shop-names.md).

Out of scope, on purpose: two ranges per day (a lunch break), hours per fulfilment method, slot capacity, per-date hour overrides, and telling the customer of a move.
