...
`validateSlotHours` refuses a save with `close_before_open` — judged on the **raw** hours, because `fulfilmentConfig` reads such a day as closed and the parsed config can never show the mistake — or `no_open_day`, the slot twin of the all-seven-days-closed refusal. `fulfilmentWarning` reports `no_slots` (slots on, dates offered, none with a slot) in red on the dashboard. See [ADR 0027](docs/adr/0027-a-fulfilment-slot-is-a-window-the-shop-names.md).
