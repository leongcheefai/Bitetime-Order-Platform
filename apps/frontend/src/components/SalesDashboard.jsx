...
export default function SalesDashboard({ lang }) {
  ...
  const getStatus = o => statuses[o.order_number] || 'pending';
  const active = orders.filter(o => getStatus(o) !== 'cancelled');

  const totalRevenue = active.reduce((s, o) => s + (o.total || 0), 0);

  const now = new Date();
  const thisMonthActive = active.filter(o => {
    const d = new Date(o.created_at);
    return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth();
  });
  const monthRevenue = thisMonthActive.reduce((s, o) => s + (o.total || 0), 0);
