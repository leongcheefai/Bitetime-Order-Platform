...
interface OrderFormProps {
  ...
  lang: Lang;
  user?: User | null;
  onSuccess: (orderNumber: string, meta: OrderSuccessMeta) => void;
  savedAddress?: SavedAddress | null;
}

type SdSlot = { label: string; cutoff: number };
type SdQuote = { for?: string; status: string; km?: number; fee?: number };
type ReferralReward = { id: string; status: string; referrerUserId?: string; giftProductId?: string; giftProductName?: string; [key: string]: any };
type AppliedReferral = { code: string; referrerId: string; referrerName: string };

export default function OrderForm({ settings, lang, user, onSuccess, savedAddress }: OrderFormProps) {
  const t = (en: string, zh: string) => lang === 'zh' ? zh : en;
  const money = (n: number | undefined | null) => Number(n || 0).toFixed(2);
  const pName = (p: Product | undefined) => (lang === 'zh' && p?.nameZh) ? p.nameZh : p?.name;
  const pDesc = (p: Product | undefined) => (lang === 'zh' && p?.descZh) ? p.descZh : p?.desc;
  ...
  const [custDateObj, setCustDateObj] = useState<Date | undefined>(undefined);
  ...
  const sameday: SamedayConfig | undefined = settings.sameday;
  const sdSlots: SdSlot[] = sameday?.slots?.length ? sameday.slots : DEFAULTS.sameday.slots;
  const nowHour = new Date().getHours();
  const slotOpen = (s: SdSlot) => nowHour < (s.cutoff ?? 24);
  ...
  const sdEligible = (p: Product) => (p as any).sameday !== false;
  ...
  const [sdQuoteRaw, setSdQuoteRaw] = useState<SdQuote | null>(null);
  useEffect(() => {
    if (mode !== 'sameday' || postcode.length !== 5 || sdQuoteRaw?.for === postcode) return;
    let cancelled = false;
    const timer = setTimeout(async () => {
      const q = await quoteSameday(postcode, sameday as SamedayConfig);
      if (cancelled) return;
  ...
  const [pendingGift, setPendingGift] = useState<ReferralReward | null>(null);
  useEffect(() => {
    if (!user || !referralEnabled) return;
    loadReferralRewards().then((rewards: ReferralReward[]) => {
      setPendingGift(rewards.find(r => r.status === 'pending' && r.referrerUserId === user!.id) ?? null);
    }).catch(() => {});
  }, [user, referralEnabled]);
  const giftProduct = pendingGift ? (settings.products.find((p: Product) => p.id === pendingGift.giftProductId) ?? null) : null;
  ...
  function getProduct(id: string): Product | undefined { return settings.products.find((p: Product) => p.id === id); }
  ...
  function promoLeft(p: any) { return Math.max(0, (p.promoLimit || 0) - promoSold(p)); }
  function hasLimit(p: any) { return (p.promoLimit || 0) > 0; }
  function hasEnd(p: any) { return !!p.promoEnd; }
  function limitOk(p: any) { return !hasLimit(p) || promoLeft(p) > 0; }
  function dateOk(p: any) { return !hasEnd(p) || new Date() <= new Date(p.promoEnd + 'T23:59:59'); }
  function promoActive(p: any) { return (p.promoPrice || 0) > 0 && (hasLimit(p) || hasEnd(p)) && limitOk(p) && dateOk(p); }
  function effPrice(p: any) { return promoActive(p) ? p.promoPrice : p.price; }
  function promoTag(p: any) {
    const parts = [];
  ...
  async function submitOrder() {
    ...
    if (mode === 'sameday') {
      const bad = Object.keys(selected).map(getProduct).filter((p): p is Product => !!p && !sdEligible(p));
      if (bad.length) {
    ...
    if (appliedVoucher) {
      ...
      const fresh = (await loadVouchers()).find((x: Voucher) => x.code === appliedVoucher.code);
    ...
    <>
      ...
      <div className="section" ref={itemsRef}>
        ...
        <div className="cookie-grid">
          {settings.products.map((p: Product) => {
