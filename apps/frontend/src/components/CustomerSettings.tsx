...
import type { Lang, Settings, Order, OrderItem } from '../types';
...
const CUST_STATUS: Record<string, { en: string; zh: string; cls: string }> = {
...
export default function CustomerSettings({ user, lang, settings = {}, onAddressSaved, refreshKey, section = 'details' }: CustomerSettingsProps) {
  const t = (en: string, zh: string) => lang === 'zh' ? zh : en;

  const [orders, setOrders] = useState<Order[]>([]);
  const [orderStatuses, setOrderStatuses] = useState<Record<string, string>>({});
  const [orderAWBs, setOrderAWBs] = useState<Record<string, string>>({});
  const [ordersLoading, setOrdersLoading] = useState(true);
  const [expandedOrders, setExpandedOrders] = useState<Record<string, boolean>>({});
  const toggleOrder = (key: string | number) => setExpandedOrders(prev => ({ ...prev, [key]: !prev[key] }));
  ...
  useEffect(() => {
    ...
    if (settings.referral?.enabled) {
      loadReferralRewards().then((all: any[]) => setMyRewards(all.filter((r: any) => r.referrerUserId === user.id))).catch(() => {});
    }
    loadVouchers().then((all: any[]) => {
      const email = user.email?.toLowerCase() ?? '';
      setMyVouchers(all.filter((v: any) => !v.email || v.email === email));
  ...
  function generateInvoice(order: Order) {
    ...
    const items = (order.items || []).map((it: OrderItem) => `
      <tr>
        ...
        <td class="num">RM ${money((it.price ?? 0) * it.qty)}</td>
      </tr>`).join('');
    const showShip = (order.mode === 'delivery' || order.mode === 'sameday') && (order.shipping_fee ?? 0) > 0;
  ...
  return (
    <div className="cust-settings">
      ...
      {(section === 'promos' || section === 'vouchers') && (() => {
        const now = new Date();
        const events = (settings.events ?? []).filter((ev: any) =>
      ...
      {(section === 'promos' || section === 'vouchers') && settings.referral?.enabled && (() => {
        ...
        const giftProductName = settings.products?.find((p: any) => p.id === settings.referral?.giftProductId)?.name || '';
        const pending = myRewards.filter((r: any) => r.status === 'pending');
        return (
          <div className="settings-section">
                      ...
                      <div className="order-accordion-left">
                        ...
                        {!expanded && order.items && order.items.length > 0 && (
                          <span className="order-accordion-preview">
                            {order.items.map((it: OrderItem) => `${it.name} ×${it.qty}`).join(', ')}
                          </span>
                        )}
                      </div>
                          ...
                          <div className="summary-section">
                            {order.items.map((item: OrderItem, j: number) => (
                              <div key={j} className="summary-row">
                                ...
                                <span>RM {(item.price ?? 0) * item.qty}</span>
                              </div>
                            ))}
                            {(order.mode === 'delivery' || order.mode === 'sameday') && (order.shipping_fee ?? 0) > 0 && (
