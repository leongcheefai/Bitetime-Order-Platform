...
export default function App() {
  ...
  const [accountSection, setAccountSection] = useState<any>(null);
  ...
  function openSection(key: any) {
    setAccountSection(key);
    setDrawerOpen(false);
  }

  function renderSuccessBox(onViewHistory: any) {
    return (
    ...
    return (
      <>
        <div className="form-wrap form-wrap--owner">
            ...
            <>
              {userPage === 'preview' && accountSection ? (
                <>
                  ...
                  <CustomerSettings user={account as any} lang={lang} settings={settings} onAddressSaved={(addr: any) => setSavedAddress(addr)} refreshKey={orderCount} section={accountSection} />
                </>
              ) : (
                <>
                  <div className="brand">
          ...
          {userPage === 'menu' && (
            <AdminPanel settings={settings} lang={lang} onSave={(newSettings: any) => setSettings(newSettings)} tab={menuTab} />
          )}

          {userPage === 'analytics' && <SalesDashboard lang={lang} />}
          {userPage === 'orders' && <OrderList key={ordersKey} lang={lang} settings={settings} {...({ user: account } as any)} />}
          {userPage === 'customers' && <CustomerList lang={lang} />}
          {userPage === 'vouchers' && <VoucherPanel lang={lang} />}
          {userPage === 'events' && <AdminPanel settings={settings} lang={lang} onSave={(newSettings: any) => setSettings(newSettings)} tab="events" />}
        </div>
        {sideDrawer}
      </>
    );
