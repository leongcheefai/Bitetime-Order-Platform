...
export default function AdminOverview() {
  ...
  return (
    <div className="dashboard-panel">
      <div className="dash-stat-grid dash-stat-grid--4">
        <StatCard label={t('Total merchants', '商家总数')} value={String(stats.total)} icon="🏪" />
        <StatCard label={t('Active', '已激活')} value={String(stats.active)} icon="✅" />
        <StatCard label={t('Pending', '待审核')} value={String(stats.pending)} icon="⏳" />
        <StatCard label={t('Suspended', '已暂停')} value={String(stats.suspended)} icon="⛔" />
      </div>
