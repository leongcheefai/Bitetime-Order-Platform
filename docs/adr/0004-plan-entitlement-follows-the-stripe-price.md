...
**A Checkout session** — cancel and resubscribe — was rejected outright. `POST /api/checkout` refuses a shop with a live subscription, and every approved basic shop has one (its cardless trial), so the "upgrade" would have to cancel first: a window where the shop is unsubscribed, a lost trial state, and a double-billing risk if the second half fails.
